...
def draw_route_with_stop(input_files: list, outputfile: str, stop_name: str, marker_image_path: str, special_stops: list, special_marker_image_path: str):
    ...
    for idx, file in enumerate(input_files):
        ...
        for x, y, name in zip(gdf.geometry.x, gdf.geometry.y, gdf["車站名稱"]):
            ...
            if name == stop_name:
                stop_found = True
                stop_location = (x, y)
            if name in special_stops:
                try:
                    img = Image.open(special_marker_image_path)
                    imagebox = OffsetImage(img, zoom=0.1)  # 調整圖片大小
                    ab = AnnotationBbox(imagebox, (x, y), frameon=False)
                    ax.add_artist(ab)
                except FileNotFoundError:
                    print(f"錯誤: 找不到標記圖片 '{special_marker_image_path}'")
...
if __name__ == "__main__":
    ...
    marker_image_path = "C:/Users/User/Documents/GitHub/cycu_oop_1132_11022343/20250429/123.png"  # 指定站牌的標記圖片
    special_marker_image_path = "C:/Users/User/Documents/GitHub/cycu_oop_1132_11022343/20250429/333.png"  # 特殊站牌的標記圖片

    # 輸入站牌名稱
    stop_name = input("請輸入站牌名稱: ")

    # 特殊站牌名稱列表
    special_stops = ["民生西路口", "和平高中"]

    # 繪製地圖
    draw_route_with_stop(input_files, outputfile, stop_name, marker_image_path, special_stops, special_marker_image_path)
